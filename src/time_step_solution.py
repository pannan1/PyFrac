...
def attempt_time_step(Frac, C, mat_properties, fluid_properties, sim_properties, inj_properties,
        ...
        if inj_properties.rate_delayed_inj_pt_func is None:
            if Frac.time >= inj_properties.injectionTime_delayed_second_injpoint:
                Qin[inj_properties.delayed_second_injpoint_elem] = inj_properties.injectionRate_delayed_second_injpoint / \
                    len(inj_properties.delayed_second_injpoint_elem)
            else:
                Qin[inj_properties.delayed_second_injpoint_elem] = inj_properties.init_rate_delayed_second_injpoint / \
                    len(inj_properties.delayed_second_injpoint_elem)
        else:
            Qin[inj_properties.delayed_second_injpoint_elem] = inj_properties.rate_delayed_inj_pt_func(
                Frac.time)/len(inj_properties.delayed_second_injpoint_elem)
        log.debug("\n  max value of the array Q(x,y) =   " + str(Qin.max()))
        log.debug("\n  Q at the delayed inj point    =   " +
                  str(Qin[inj_properties.delayed_second_injpoint_elem]))
    ...
    while norm > sim_properties.tolFractFront:
        ...
        if exitstatus == 1:
            ...
            norm = abs((sum(Fr_k.FillF) - sum(fill_frac_last)) /
                       len(Fr_k.FillF))
        else:
            norm = np.nan
        ...
        if norm is not np.nan:
            if abs((previous_norm-norm)/norm) < 0.001:
                log.debug('Norm of subsequent Norms of subsequent filling fraction estimates = ' +
                          str(abs((previous_norm-norm)/norm)) + ' < 0.001')
                exitstatus = 15
    ...
    log.debug("Fracture front converged after " + repr(k) +
              " iterations with norm = " + repr(norm))
...
def injection_same_footprint(Fr_lstTmStp, C, timeStep, Qin, mat_properties, fluid_properties, sim_properties,
    ...
    if sum(mat_properties.Cprime[Fr_lstTmStp.EltCrack]) > 0.:
        ...
        t_lst_min_t0 = Fr_lstTmStp.time - \
            Fr_lstTmStp.Tarrival[Fr_lstTmStp.EltChannel]
    ...
    if sim_properties.doublefracture and Fr_lstTmStp.fronts_dictionary['number_of_fronts'] == 2:
        ...
        doublefracturedictionary = {
            "number_of_fronts": Fr_lstTmStp.fronts_dictionary['number_of_fronts']}
    ...
    Fr_kplus1.pNet[Fr_lstTmStp.EltCrack] = p_k[Fr_lstTmStp.EltCrack] - \
        mat_properties.SigmaO[Fr_lstTmStp.EltCrack]
    ...
    if len(return_data) > 3:
        Fr_kplus1.injectionRate = np.zeros(
            Fr_kplus1.mesh.NumberOfElts, dtype=np.float64)
        ...
        Fr_kplus1.source = Fr_lstTmStp.EltCrack[np.where(
            Qin[Fr_lstTmStp.EltCrack] > 0)[0]]
        Fr_kplus1.sink = Fr_lstTmStp.EltCrack[np.where(
            Qin[Fr_lstTmStp.EltCrack] < 0)[0]]

    if fluid_properties.turbulence:
        if sim_properties.saveReynNumb or sim_properties.saveFluidFlux:
            ReNumb, check = turbulence_check_tip(
                fluidVel, Fr_kplus1, fluid_properties, return_ReyNumb=True)
            if sim_properties.saveReynNumb:
                Fr_kplus1.ReynoldsNumber = ReNumb
            if sim_properties.saveFluidFlux:
                Fr_kplus1.fluidFlux = ReNumb * 3 / 4 / \
                    fluid_properties.density * fluid_properties.viscosity
        if sim_properties.saveFluidVel:
            Fr_kplus1.fluidVelocity = fluidVel
        if sim_properties.saveFluidVelAsVector:
            raise SystemExit('saveFluidVelAsVector Not yet implemented')
        if sim_properties.saveFluidFluxAsVector:
            raise SystemExit('saveFluidFluxAsVector Not yet implemented')
    else:
        if sim_properties.saveFluidFlux or sim_properties.saveFluidVel or sim_properties.saveReynNumb or sim_properties.saveFluidFluxAsVector or sim_properties.saveFluidVelAsVector:
            ...
            if sim_properties.saveFluidFlux:
                fflux = np.zeros(
                    (4, Fr_kplus1.mesh.NumberOfElts), dtype=np.float32)
                fflux[:, Fr_kplus1.EltCrack] = fluid_flux
                Fr_kplus1.fluidFlux = fflux

            if sim_properties.saveFluidFluxAsVector:
                fflux_components = np.zeros(
                    (8, Fr_kplus1.mesh.NumberOfElts), dtype=np.float32)
...
def injection_extended_footprint(w_k, Fr_lstTmStp, C, timeStep, Qin, mat_properties, fluid_properties,
    ...
    while itr < sim_properties.maxProjItrs:
        ...
        # Initializing the cells with extremely
        sgndDist_k = 1e50 * np.ones((Fr_lstTmStp.mesh.NumberOfElts,), float)
        ...
        current_prefactor = sim_properties.get_time_step_prefactor(
            Fr_lstTmStp.time + timeStep)
        ...
        norm = np.linalg.norm(
            abs(alpha_ribbon_k - alpha_ribbon_km1) / np.pi * 2)
    ...
    if sim_properties.projMethod == 'ILSA_orig':
        ...
        while not correct_size_of_pstv_region[0]:
            ...
            if correct_size_of_pstv_region[1]:
                ...
                # !!! EltsTipNew are the intersection between the fictitius cells and the frontlist as tip in order to decide the direction of remeshing
                Fr_kplus1.EltTip = EltsTipNew
                # (in case of anisotropic remeshing)
                # You are here because the level set has negative values until the end of the mesh
                exitstatus = 12
                # or because a fictitius cell has intersected the mesh.frontlist
                return exitstatus, Fr_kplus1
            ...
            if not correct_size_of_pstv_region[0]:
                ...
                front_region = np.unique(np.ndarray.flatten(
                    Fr_lstTmStp.mesh.NeiElements[front_region]))

                # the search region outwards from the front position at last time step
                pstv_region = np.where(Fr_lstTmStp.sgndDist[front_region] >= -(Fr_lstTmStp.mesh.hx ** 2 +
                                                                               Fr_lstTmStp.mesh.hy ** 2) ** 0.5)[0]
                # the search region inwards from the front position at last time step
                ngtv_region = np.where(
                    Fr_lstTmStp.sgndDist[front_region] < 0)[0]
    ...
    Vel_k = -(sgndDist_k[EltsTipNew] -
              Fr_lstTmStp.sgndDist[EltsTipNew]) / timeStep
    ...
    partlyFilledTip = np.arange(EltsTipNew.shape[0])[
        np.in1d(EltsTipNew, EltTip_k)]
    ...
    if sim_properties.projMethod != 'LS_continousfront':
        ...
        zrVrtx_newTip = find_zero_vertex(
            EltsTipNew, sgndDist_k, Fr_lstTmStp.mesh)
    else:
    ...
    if mat_properties.TI_elasticity:
        ...
        Eprime_tip = np.full((EltsTipNew.size,),
                             mat_properties.Eprime, dtype=np.float64)
    ...
    smallNgtvWTip = np.where(np.logical_and(
        wTip < 0, wTip > -1e-4 * np.mean(wTip)))
    ...
    if sum(mat_properties.Cprime[EltsTipNew]) > 0:
        ...
        LkOff[EltsTipNew] = 2 * mat_properties.Cprime[EltsTipNew] * Integral_over_cell(EltsTipNew,
                                                                                       ...
                                                                                       Vel=Vel_k,
                                                                                       dt=timeStep,
                                                                                       arrival_t=Fr_lstTmStp.TarrvlZrVrtx[
                                                                                           EltsTipNew])
    ...
    t_enter = Fr_lstTmStp.time + timeStep - \
        l_k[new_channel] / Vel_k[new_channel]
    max_l = Fr_lstTmStp.mesh.hx * \
        np.cos(alpha_k[new_channel]) + Fr_lstTmStp.mesh.hy * \
        np.sin(alpha_k[new_channel])
    t_leave = Fr_lstTmStp.time + timeStep - \
        (l_k[new_channel] - max_l) / Vel_k[new_channel]
    Tarrival_k[EltsTipNew[new_channel]] = (t_enter + t_leave) / 2
    to_correct = np.where(
        Tarrival_k[EltsTipNew[new_channel]] < max_Tarrival)[0]
    ...
    Fr_kplus1.pNet = np.zeros((Fr_kplus1.mesh.NumberOfElts,))
    Fr_kplus1.pNet[EltCrack_k] = pf_n_plus1[EltCrack_k] - \
        mat_properties.SigmaO[EltCrack_k]
    ...
    if sim_properties.projMethod != 'LS_continousfront':
        ...
        if sim_properties.saveToDisk and sim_properties.saveStatisticsPostCoalescence and Fr_lstTmStp.number_of_fronts != Fr_kplus1.number_of_fronts:
            ...
            append_to_json_file(myJsonName, Fr_kplus1.mesh.nx,
                                'append2keyAND2list', key='nx')
            append_to_json_file(myJsonName, Fr_kplus1.mesh.ny,
                                'append2keyAND2list', key='ny')
            append_to_json_file(myJsonName, Fr_kplus1.mesh.hx,
                                'append2keyAND2list', key='hx')
            append_to_json_file(myJsonName, Fr_kplus1.mesh.hy,
                                'append2keyAND2list', key='hy')
            append_to_json_file(myJsonName, Fr_kplus1.EltCrack.size,
                                'append2keyAND2list', key='elements_in_crack')
            append_to_json_file(myJsonName, Fr_kplus1.EltTip.size,
                                'append2keyAND2list', key='elements_in_tip')
            append_to_json_file(myJsonName, Fr_kplus1.time,
                                'append2keyAND2list', key='coalescence_time')

    Fr_kplus1.FractureVolume = np.sum(Fr_kplus1.w) * Fr_kplus1.mesh.EltArea
    Fr_kplus1.Tarrival = Tarrival_k
    new_tip = np.where(np.isnan(Fr_kplus1.TarrvlZrVrtx[Fr_kplus1.EltTip]))[0]
    Fr_kplus1.TarrvlZrVrtx[Fr_kplus1.EltTip[new_tip]
    ...
    Fr_kplus1.injectedVol += sum(Qin) * timeStep
    Fr_kplus1.efficiency = (Fr_kplus1.injectedVol -
                            Fr_kplus1.LkOffTotal) / Fr_kplus1.injectedVol
...
def solve_width_pressure(Fr_lstTmStp, sim_properties, fluid_properties, mat_properties, inj_properties, EltTip,
                         ...
                         Vel, corr_ribbon, doublefracturedictionary=None):
    """
    ...
    if sim_properties.get_volumeControl():
        ...
        if sim_properties.symmetric and not sim_properties.useBlockToeplizCompression:
            ...
            for i in range(len(EltTip_sym)):
                if len(np.where(EltTip == wTip_sym_elts[i])[0]) != 1:
                    other_corr = get_symetric_elements(
                        Fr_lstTmStp.mesh, [wTip_sym_elts[i]])
                    for j in range(4):
            ...
            if sim_properties.doublefracture and doublefracturedictionary['number_of_fronts'] == 2:
                ...
                EltChannelFracture0 = np.setdiff1d(
                    Fr_lstTmStp.fronts_dictionary['crackcells_0'], Fr_lstTmStp.fronts_dictionary['TIPcellsONLY_0'])
                EltChannelFracture1 = np.setdiff1d(
                    Fr_lstTmStp.fronts_dictionary['crackcells_1'], Fr_lstTmStp.fronts_dictionary['TIPcellsONLY_1'])
        ...
        perfNode_nonLinSys = instrument_start(
            'nonlinear system solve', perfNode)
        perfNode_widthConstrItr = instrument_start(
            'width constraint iteration', perfNode_nonLinSys)
        perfNode_linSys = instrument_start(
            'linear system solve', perfNode_widthConstrItr)
        status = True
        fail_cause = None
        try:
        ...
        if perfNode is not None:
            ...
            instrument_close(perfNode_nonLinSys, perfNode_widthConstrItr, None,
                             len(b), status, fail_cause, Fr_lstTmStp.time)
            perfNode_nonLinSys.widthConstraintItr_data.append(
                perfNode_widthConstrItr)

            instrument_close(perfNode, perfNode_nonLinSys, None, len(
                b), status, fail_cause, Fr_lstTmStp.time)
        ...
        if sim_properties.symmetric:
            del_w = np.zeros((Fr_lstTmStp.mesh.NumberOfElts,),
                             dtype=np.float64)
            ...
            w = np.copy(Fr_lstTmStp.w)
            if sim_properties.doublefracture and doublefracturedictionary['number_of_fronts'] == 2:
                w[EltChannelFracture0] += sol[np.arange(
                    EltChannelFracture0.size)]
                w[EltChannelFracture1] += sol[np.arange(
                    EltChannelFracture0.size, EltChannelFracture0.size+EltChannelFracture1.size)]
                w[EltTipFracture0] = wTipFR0
                w[EltTipFracture1] = wTipFR1
            else:
                w[Fr_lstTmStp.EltChannel] += sol[np.arange(
                    Fr_lstTmStp.EltChannel.size)]
                w[EltTip] = wTip
    ...
    if sim_properties.get_viscousInjection():
        ...
        # only taking channel elements to solve
        ...
        while active_contraint:
            ...
            _, comm_neg, comm_to_impose = np.intersect1d(
                neg, to_impose, return_indices=True)
            ...
            corr_nei = np.full((len(EltCrack_k), 4),
                               len(EltCrack_k), dtype=int)
            for i, elem in enumerate(EltCrack_k):
                corresponding = np.where(
                    EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 0])[0]
                if len(corresponding) > 0:
                    corr_nei[i, 0] = corresponding
                corresponding = np.where(
                    EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 1])[0]
                if len(corresponding) > 0:
                    corr_nei[i, 1] = corresponding
                corresponding = np.where(
                    EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 2])[0]
                if len(corresponding) > 0:
                    corr_nei[i, 2] = corresponding
                corresponding = np.where(
                    EltCrack_k == Fr_lstTmStp.mesh.NeiElements[elem, 3])[0]
                if len(corresponding) > 0:
            ...
            pf_guess_neg = np.dot(
                C[np.ix_(neg, EltCrack_k)], w_guess[EltCrack_k]) + mat_properties.SigmaO[neg]
            pf_guess_tip = np.dot(C[np.ix_(to_impose_k, EltCrack_k)],
                                  w_guess[EltCrack_k]) + mat_properties.SigmaO[to_impose_k]
            ...
            if sim_properties.elastohydrSolver == 'implicit_Picard' or sim_properties.elastohydrSolver == 'implicit_Anderson':
                ...
                if inj_properties.modelInjLine:
                    inj_cells = np.intersect1d(
                        inj_properties.sourceElem, Fr_lstTmStp.EltChannel)
                    sink_cells = np.intersect1d(np.asarray(
                        inj_properties.sinkElem, dtype=int), Fr_lstTmStp.EltCrack)

                    sink = np.zeros(Fr_lstTmStp.mesh.NumberOfElts)
                    if inj_properties.sinkElem:
                        sink[inj_properties.sinkElem] = inj_properties.sinkVel * \
                            Fr_lstTmStp.mesh.EltArea

                    indxCurTime = max(
                        np.where(Fr_lstTmStp.time + timeStep >= inj_properties.injectionRate[0, :])[0])
                    # current injection rate
                    currentRate = inj_properties.injectionRate[1, indxCurTime]
                ...
                raise SystemExit(
                    "The given elasto-hydrodynamic solver is not supported!")
            ...
            if len(below_wc_k) > 0:
                ...
                wHst_above_wc = np.where(
                    Fr_lstTmStp.wHist[to_solve_k] >= mat_properties.wc)[0]
                impose_wc_at = np.intersect1d(wHst_above_wc, below_wc_k)

                # for cells with max width in w history less than wc
                wHst_below_wc = np.where(
                    Fr_lstTmStp.wHist[to_solve_k] < mat_properties.wc)[0]
                dwdt_neg = np.where(
                    w[to_solve_k] <= Fr_lstTmStp.w[to_solve_k])[0]
                impose_wHist_at = np.intersect1d(wHst_below_wc, dwdt_neg)

                neg_k = to_solve_k[np.concatenate(
                    (impose_wc_at, impose_wHist_at))]
                # the corresponding values of width to be imposed in cells where width constraint is active
                wc_k = np.full((len(impose_wc_at) + len(impose_wHist_at),),
                               mat_properties.wc, dtype=np.float64)
                ...
                if len(new_neg) == 0:
                    ...
                    log.debug(
                        'Iterating on cells with active width constraint...')
            else:
                active_contraint = False
            ...
            pf_k[to_solve_k] = np.dot(
                C[np.ix_(to_solve_k, EltCrack)], w[EltCrack]) + mat_properties.SigmaO[to_solve_k]
            if sim_properties.solveDeltaP:
                pf_k[neg_km1] = Fr_lstTmStp.pFluid[neg_km1] + \
                    sol[len(to_solve_k):len(to_solve_k) + len(neg_km1)]
                pf_k[to_impose_k] = Fr_lstTmStp.pFluid[to_impose_k] + sol[len(to_solve_k) + len(neg_km1):
                                                                          len(to_solve_k) + len(neg_km1) + len(to_impose_k)]
            else:
                pf_k[neg_km1] = sol[len(to_solve_k):len(
                    to_solve_k) + len(neg_km1)]
        ...
        if perfNode_nonLinSys is not None:
            instrument_close(perfNode, perfNode_nonLinSys, None,
                             len(sol), True, None, Fr_lstTmStp.time)
            perfNode.nonLinSolve_data.append(perfNode_nonLinSys)
        ...
        if inj_properties.modelInjLine:
            ...
            Q[inj_act] = sol[pil_indx + 1 +
                             len(inj_ch): pil_indx + 1 + len(inj_ch) + len(inj_act)]
...
def turbulence_check_tip(vel, Fr, fluid, return_ReyNumb=False):
    ...
    wLftEdge = (Fr.w[Fr.EltRibbon] +
                Fr.w[Fr.mesh.NeiElements[Fr.EltRibbon, 0]]) / 2
    wRgtEdge = (Fr.w[Fr.EltRibbon] +
                Fr.w[Fr.mesh.NeiElements[Fr.EltRibbon, 1]]) / 2
    wBtmEdge = (Fr.w[Fr.EltRibbon] +
                Fr.w[Fr.mesh.NeiElements[Fr.EltRibbon, 2]]) / 2
    wTopEdge = (Fr.w[Fr.EltRibbon] +
                Fr.w[Fr.mesh.NeiElements[Fr.EltRibbon, 3]]) / 2

    Re = np.zeros((4, Fr.EltRibbon.size,), dtype=np.float64)
    Re[0, :] = 4 / 3 * fluid.density * wLftEdge * \
        vel[0, Fr.EltRibbon] / fluid.viscosity
    Re[1, :] = 4 / 3 * fluid.density * wRgtEdge * \
        vel[1, Fr.EltRibbon] / fluid.viscosity
    Re[2, :] = 4 / 3 * fluid.density * wBtmEdge * \
        vel[2, Fr.EltRibbon] / fluid.viscosity
    Re[3, :] = 4 / 3 * fluid.density * wTopEdge * \
        vel[3, Fr.EltRibbon] / fluid.viscosity
    ...
    if return_ReyNumb:
        wLftEdge = (Fr.w[Fr.EltCrack] +
                    Fr.w[Fr.mesh.NeiElements[Fr.EltCrack, 0]]) / 2
        wRgtEdge = (Fr.w[Fr.EltCrack] +
                    Fr.w[Fr.mesh.NeiElements[Fr.EltCrack, 1]]) / 2
        wBtmEdge = (Fr.w[Fr.EltCrack] +
                    Fr.w[Fr.mesh.NeiElements[Fr.EltCrack, 2]]) / 2
        wTopEdge = (Fr.w[Fr.EltCrack] +
                    Fr.w[Fr.mesh.NeiElements[Fr.EltCrack, 3]]) / 2

        Re = np.zeros((4, Fr.mesh.NumberOfElts,), dtype=np.float64)
        Re[0, Fr.EltCrack] = 4 / 3 * fluid.density * \
            wLftEdge * vel[0, Fr.EltCrack] / fluid.viscosity
        Re[1, Fr.EltCrack] = 4 / 3 * fluid.density * \
            wRgtEdge * vel[1, Fr.EltCrack] / fluid.viscosity
        Re[2, Fr.EltCrack] = 4 / 3 * fluid.density * \
            wBtmEdge * vel[2, Fr.EltCrack] / fluid.viscosity
        Re[3, Fr.EltCrack] = 4 / 3 * fluid.density * \
            wTopEdge * vel[3, Fr.EltCrack] / fluid.viscosity
...
def time_step_explicit_front(Fr_lstTmStp, C, timeStep, Qin, mat_properties, fluid_properties, sim_properties,
    ...
    # Initializing the cells with maximum
    sgndDist_k = 1e50 * np.ones((Fr_lstTmStp.mesh.NumberOfElts,), float)
    ...
    # expected range of possible propagation
    expected_range = max(current_prefactor * 12.66 *
                         cell_diag, 1.5 * cell_diag)
    ...
    if sum(mat_properties.Cprime[Fr_lstTmStp.EltChannel]) > 0:
        t_since_arrival = Fr_lstTmStp.time - \
            Fr_lstTmStp.Tarrival[Fr_lstTmStp.EltChannel]
    ...
    Fr_kplus1.v = -(sgndDist_k[Fr_kplus1.EltTip] -
                    Fr_lstTmStp.sgndDist[Fr_kplus1.EltTip]) / timeStep
    Fr_kplus1.sgndDist = sgndDist_k
    Fr_kplus1.sgndDist_last = Fr_lstTmStp.sgndDist
    Fr_kplus1.timeStep_last = timeStep
    new_tip = np.where(np.isnan(Fr_kplus1.TarrvlZrVrtx[Fr_kplus1.EltTip]))[0]
